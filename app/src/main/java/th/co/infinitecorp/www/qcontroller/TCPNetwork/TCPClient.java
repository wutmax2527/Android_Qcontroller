...
public class TCPClient implements TCPClientService {
    ...
    public boolean CloseSocket() {
        try {
            ...
            Log.d(TAG, "Close Socket Error=" + e.getLocalizedMessage());
        }
        return false;
    }
        ...
        protected Void doInBackground(Void... voids) {
            ...
            try {
                ...
                while (taskState & socket.isConnected()) {
                    ...
                    if (in.ready()) {
                        int charsRead = 0;
                        final char[] buffer = new char[2024]; //choose your buffer size if you need other than 1024
                ...
                try {
                    ...
                    while (socket.isConnected()) {
                        ...
                        if (in.ready()) {
                            ...
                            if (len > 0) {
                                incomingMsg = new String(bytes).substring(0, len);
                                final byte[] rBuf = bytes;
                                final String msg = incomingMsg;
                                new Handler(Looper.getMainLooper()).post(new Runnable() {
                                    public void run() {
                                        tcpClientService.getDataReceivedListener().onDataReceived(socket, msg, rBuf);
                                    }
                                });
                                isRunning = false;
                                break;
